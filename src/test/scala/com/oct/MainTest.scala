...
class MainTest extends FunSuite with BeforeAndAfter with Matchers {
  ...
  test("builds composite using some fucking resemblance to an inbred genetic algorithm") {
    ...
    val imgDevilsThumb = Image.fromFile(files.filter(f => f.getAbsolutePath.contains("0010-2015-07-1112-24-27")).head).scale(0.5, ScaleMethod.FastScale)
    val imgWetMtns = Image.fromFile(files.filter(f => f.getAbsolutePath.contains("0068-2014-11-2816-57-05")).head)

    val otherImages = files.tail.map(f => Image.fromFile(f).scale(0.125, ScaleMethod.FastScale))
    val emptyImage = GetTransparentImage(imgDevilsThumb.width, imgDevilsThumb.height)
